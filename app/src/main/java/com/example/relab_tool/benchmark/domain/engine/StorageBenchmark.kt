...
class StorageBenchmark(private val context: Context) : BenchmarkEngine {
    ...
    override suspend fun run(onProgress: suspend (Float) -> Unit): List<SubScore> =
        withContext(Dispatchers.IO) {
            ...
            onProgress(0.02f)
            val seqWriteVal = BenchmarkHarness.medianOfThree(warmups = 3) { runSequentialWrite(1024) }
            results += subScore("IO_01: Sequential Write (1024MB)", seqWriteVal, "MB/s", 500.0, 12000.0, false)

            // IO_02 — Sequential read (NIO FileChannel, 256MB)
            onProgress(0.07f)
            val seqReadVal = BenchmarkHarness.medianOfThree(warmups = 3) { runSequentialRead(1024) }
            results += subScore("IO_02: Sequential Read (1024MB)", seqReadVal, "MB/s", 800.0, 21000.0, false)
            ...
            results += subScore("IO_03: 1MB Block Write (64 blocks)", blockWriteVal, "MB/s", 450.0, 10500.0, false)

            // IO_04 — Read throughput 1MB blocks
            onProgress(0.17f)
            val blockReadVal = BenchmarkHarness.medianOfThree(warmups = 3) { runBlockRead(1024 * 1024, 64) }
            results += subScore("IO_04: 1MB Block Read (64 blocks)", blockReadVal, "MB/s", 700.0, 18000.0, false)

            // IO_05 — Random 4KB write (IOPS)
            onProgress(0.22f)
            val rand4kWriteVal = BenchmarkHarness.medianOfThree(warmups = 3) { runRandom4KWrite(50_000) }
            results += subScore("IO_05: Random 4KB Write IOPS", rand4kWriteVal, "IOPS", 20_000.0, 350_000.0, false)

            // IO_06 — Random 4KB read (IOPS)
            onProgress(0.27f)
            val rand4kReadVal = BenchmarkHarness.medianOfThree(warmups = 3) { runRandom4KRead(50_000) }
            ...
            // IO_08 — SQLite WAL sequential insert (200K rows)
            onProgress(0.37f)
            val sqlSeqVal = BenchmarkHarness.medianOfThree(warmups = 3) { runSqliteWalInsert(200_000) }
            results += subScore("IO_08: SQLite WAL Insert (200K)", sqlSeqVal, "ms", 2500.0, 66.667, true)

            // IO_09 — SQLite bulk query (SELECT + sort 200K rows)
            onProgress(0.42f)
            val sqlQueryVal = BenchmarkHarness.medianOfThree(warmups = 3) { runSqliteQuery(200_000) }
            results += subScore("IO_09: SQLite Query (200K rows)", sqlQueryVal, "ms", 1500.0, 33.333, true)

            // IO_10 — SQLite indexed query (index scan vs full scan delta)
            onProgress(0.47f)
            val sqlIndexVal = BenchmarkHarness.medianOfThree(warmups = 3) { runSqliteIndexScan(200_000) }
            results += subScore("IO_10: SQLite Index Scan (200K)", sqlIndexVal, "ms", 400.0, 6.667, true)
            ...
            results += subScore("IO_11: O_DIRECT Write (128MB)", directWriteVal, "MB/s", 450.0, 12000.0, false)

            // IO_12 — O_DIRECT sequential read
            onProgress(0.57f)
            val directReadVal = BenchmarkHarness.medianOfThree(warmups = 3) { runODirectRead(128) }
            results += subScore("IO_12: O_DIRECT Read (128MB)", directReadVal, "MB/s", 700.0, 21000.0, false)

            // IO_13 — MappedByteBuffer sequential read (mmap)
            onProgress(0.62f)
            val mmapVal = BenchmarkHarness.medianOfThree(warmups = 3) { runMmapRead(128) }
            results += subScore("IO_13: MappedByteBuffer Read", mmapVal, "MB/s", 800.0, 21000.0, false)
            ...
            results += subScore("IO_14: File Open/Close Latency", openCloseVal, "ms/op", 0.6, 0.010, true)

            // IO_15 — Directory listing (10K files)
            onProgress(0.70f)
            val dirListVal = BenchmarkHarness.medianOfThree(warmups = 3) { runDirectoryListing(10_000) }
            results += subScore("IO_15: Directory Listing (10K)", dirListVal, "ms", 450.0, 6.667, true)

            // IO_16 — File metadata stat() (1000 files)
            onProgress(0.75f)
            val statVal = BenchmarkHarness.medianOfThree(warmups = 3) { runFileStatOps(1000) }
            results += subScore("IO_16: File Stat (1K files)", statVal, "µs/op", 80.0, 1.000, true)
            ...
            results += subScore("IO_17: SharedPrefs Simulation", sharedPrefVal, "ms", 400.0, 6.667, true)
            ...
            results += subScore("IO_18: Scoped Storage Overhead", scopedVal, "MB/s", 250.0, 6000.0, false)
            ...
            results += subScore("IO_19: JSON Serialize to Disk", jsonDiskVal, "ms", 1500.0, 33.333, true)

            // IO_20 — File integrity verification (SHA-256 of 128MB file)
            onProgress(0.97f)
            val integrityVal = BenchmarkHarness.medianOfThree(warmups = 3) { runFileIntegrityCheck() }
            results += subScore("IO_20: File Integrity Check (128MB SHA)", integrityVal, "MB/s", 600.0, 15000.0, false)
    ...
    private fun runSequentialRead(sizeMb: Int): Double {
        ...
        writePatternFile(file, sizeMb)
        BenchmarkNativeBridge.safeEvictCache(file.absolutePath)
        return try {
    ...
    private fun runBlockWrite(blockSize: Int, blockCount: Int): Double {
        ...
        return try {
            ...
            val elapsed = (System.nanoTime() - start) / 1e9
            (blockSize.toLong() * blockCount / 1024.0 / 1024.0) / elapsed
        } finally {
            file.delete()
        }
    }
    ...
    private fun runBlockRead(blockSize: Int, blockCount: Int): Double {
        ...
        writePatternFile(file, blockSize.toLong() * blockCount)
        BenchmarkNativeBridge.safeEvictCache(file.absolutePath)
        return try {
    ...
    private fun runRandom4KRead(opCount: Int): Double {
        ...
        writePatternFile(file, fileSize)
        BenchmarkNativeBridge.safeEvictCache(file.absolutePath)
        return try {
            val blockSize = 4096
            val buf = ByteArray(blockSize)
    ...
    private fun runODirectWrite(sizeMb: Int): Double {
        ...
        return try {
            ...
            if (result > 0.0) result else runSequentialWrite(sizeMb)
        } finally {
            file.delete()
        }
    }
    ...
    private fun runODirectRead(sizeMb: Int): Double {
        ...
        return try {
            ...
            if (result > 0.0) result else runSequentialRead(sizeMb)
        } finally {
            file.delete()
        }
    }
    ...
    private fun runMmapRead(sizeMb: Int): Double {
        ...
        BenchmarkNativeBridge.safeEvictCache(file.absolutePath)
        return try {
            val buf = ByteArray(1024 * 1024) // 1MB read buffer
            val start = System.nanoTime()
            ...
            while (mapped.hasRemaining()) {
                val toRead = minOf(buf.size, mapped.remaining())
                mapped.get(buf, 0, toRead)
                sum += buf[0].toLong()
            }
    ...
    private fun writePatternFile(file: File, sizeMb: Int) {
        writePatternFile(file, sizeMb.toLong() * 1024L * 1024L)
    }

    private fun writePatternFile(file: File, sizeBytes: Long) {
        ...
        RandomAccessFile(file, "rw").use { raf ->
            ...
            while (remaining > 0L) {
                val bytesThisRound = minOf(chunk.size.toLong(), remaining).toInt()
                val buf = ByteBuffer.wrap(chunk, 0, bytesThisRound)
                while (buf.hasRemaining()) channel.write(buf)
